---
version: 1.2.0
x-collection-name: Meetup OpenEvents (stream)
info:
  title: Meetup
  version: 1.2.0
servers:
- url: api.meetup.com/2/open_events
  scheme: https
  asyncapi_servers_variables:
    ...
    text:
      description: Events that contain the given term or terms somewhere in their
        content. The terms are OR'd by default. Separate terms with " AND " for events
        that have combined terms. To have terms automatically AND'd, set the "and_text"
        to true
    category:
    ...
    lon:
      description: A valid longitude, limits the returned group events to those within
        radius miles
    time:
...
stream:
  framing:
    ...
    delimiter: \r\n
  read:
  - $ref: '#/components/messages/2_open_events'
---